...
func NewRoleService(repo domain.RoleRepository, cacheClient cache.Client, cacheTTL time.Duration, log *zap.Logger) domain.RoleService {
	return &roleService{repo: repo, cache: cacheClient, cacheTTL: cacheTTL, log: log}
}
...
func (s *roleService) GetAll(ctx context.Context, pq *pagination.PaginationQuery) ([]domain.RoleResponse, *response.PaginationMeta, error) {
	cacheKey := roleListCacheKey(pq)
	var cached roleListCacheEntry
	if hit, err := s.cache.Get(ctx, cacheKey, &cached); err == nil && hit {
		return cached.Roles, cached.Meta, nil
	} else if err != nil {
		s.log.Warn("failed to read roles from cache", zap.String("key", cacheKey), zap.Error(err))
	}
	...
	if err := s.cache.Set(ctx, cacheKey, roleListCacheEntry{Roles: resp, Meta: meta}, s.cacheTTL); err != nil {
		s.log.Warn("failed to store roles in cache", zap.String("key", cacheKey), zap.Error(err))
	}
...
func (s *roleService) GetByID(ctx context.Context, id string) (*domain.RoleResponse, error) {
	cacheKey := roleDetailCacheKey(id)
	var cached domain.RoleResponse
	if hit, err := s.cache.Get(ctx, cacheKey, &cached); err == nil && hit {
		return &cached, nil
	} else if err != nil {
		s.log.Warn("failed to read role detail from cache", zap.String("key", cacheKey), zap.Error(err))
	}
	...
	if err := s.cache.Set(ctx, cacheKey, resp, s.cacheTTL); err != nil {
		s.log.Warn("failed to store role detail in cache", zap.String("key", cacheKey), zap.Error(err))
	}
...
func (s *roleService) RemovePermissions(ctx context.Context, id string, req *domain.AssignPermissionsRequest) error {
	...
	if err := s.repo.RemovePermissions(ctx, id, req.PermissionIDs); err != nil {
		return err
	}
...
type roleListCacheEntry struct {
	Roles []domain.RoleResponse    `json:"roles"`
	Meta  *response.PaginationMeta `json:"meta"`
}

func roleListCacheKey(pq *pagination.PaginationQuery) string {
	return fmt.Sprintf(
		"roles:list:page=%d:per_page=%d:sort=%s:order=%s:search=%s",
		pq.Page,
		pq.PerPage,
		pq.Sort,
		pq.Order,
...
func roleDetailCacheKey(id string) string {
	return fmt.Sprintf("roles:detail:%s", id)
}

func (s *roleService) invalidateRoleCache(ctx context.Context) {
